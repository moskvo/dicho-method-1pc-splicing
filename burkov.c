...
void notrecursive_treesolver ( node_t* root, WITH_SPLICETYPE(knint cons) ){
...
void recursive_treesolver(node_t* root, WITH_SPLICETYPE(knint cons)){
  if( root->length != 0 ) return;

  recursive_treesolver (root->lnode,WITH_SPLICEVAR(cons));
  recursive_treesolver (root->rnode,WITH_SPLICEVAR(cons));

  if ( root->lnode->length > root->rnode->length )
  	dichosolve(root, root->lnode, root->rnode, WITH_SPLICEVAR(cons) );
  else
  	dichosolve(root, root->rnode, root->lnode, WITH_SPLICEVAR(cons) );
//  print_tree(root);
}

void move_bignode ( node_t* to,  node_t* big, node_t* small ) {
  to->items = big->items;
...
void copy_bignode ( node_t* to,  node_t* big, node_t* small ) {
	to->items = copyitemslist ( big->length, big->items );
	to->length = big->length;
}
...
void dichosolve ( node_t* to, node_t* big, node_t* small, WITH_SPLICETYPE(knint cons) ) {
  ...
  if ( (tmp = find_preplace_badcutter_simple(lastelem, fp->w, &(to->length))) == NULL ) { // if we must put item with fp->w weight to first place
		if ( to->items->w == fp->w ) {
			...
			sp = copyitem(fp);
			/*if ( fp->flag != TRUE_ELEM ) */sp->flag = OLD_ELEM;
			to->items = sp;
			to->items->next = lastelem;
			lastelem = to->items;
			to->length++;
		...
		sp = copyitem(fp);
		// if item inserted
		if ( put_item_simple (tmp, &sp, &(to->length)) == 0 ) {
			/*if ( fp->flag != TRUE_ELEM ) */sp->flag = OLD_ELEM;
			lastelem = sp;
		} else { // put_item drops sp
			lastelem = tmp;
			free_items(&sp);
		}
  }
  //puts ("cycle 'put new elements of second table ...'"); fflush(stdout);
  for( fp = fp->next ; fp != NULL /*&& *(fp->w) <= cons*/ ; fp = fp->next ) {
    lastelem = find_preplace_badcutter_simple (lastelem, fp->w, &(to->length));
		sp = copyitem(fp);
    if ( put_item_simple (lastelem, &sp, &(to->length)) == 0 ) {
    	/*if ( fp->flag != TRUE_ELEM ) */sp->flag = OLD_ELEM;
    } else {
			free_items(&sp);
    }
 ...
 #endif
	//puts("end dischosolve");	fflush(stdout);
} // dichosolve()
